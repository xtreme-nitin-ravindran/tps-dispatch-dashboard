...
- Promotion fast-forwards dev to the resulting main merge when possible and lets the configured branch-based Pages publisher deploy main automatically; it does not submit a duplicate build request. Before starting another change, run `git switch dev` and `git pull --ff-only origin dev`. If main advanced independently, merge `origin/main` into dev and push again to rerun checks.
