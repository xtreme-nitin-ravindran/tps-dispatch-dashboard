...
export function renderDisruptions(data, origin, radius, map) {
  ...
  if (map && showMap) {
    ...
    for (const item of roads) {
      ...
      const layer=item.line?.length>1 ? L.polyline(item.line,{color:'#f28cba',weight:5,opacity:.8}) : item.coordinates ? L.circleMarker(item.coordinates,{color:'#f28cba',radius:6}) : null;
